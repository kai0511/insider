...
```{r, fig.height=4, fig.width = 12, size="small"}
loadfonts(quiet = T)

structure_names <- c("TC left", "FWM right", "FWM left", "HPC right", "PC right", "TC right", "HPC left", "PC left")

# result <- as.data.frame(down_reg)
# selected_result <- d[c(11, 16, 24, 29, 30),c(2,8)]

# selected <- unique(unlist(strsplit(selected_result[,2], split = "/")))

# selected <- c("PLEKHG5", "NCS1", "GRIK5", "NRGN")
selected <- c("NRGN", "CAMK2A", "SHISA6")

# inter_profiles <- interactions %*% column_factor
# inter_var <- as.vector(apply(inter_profiles, 2, var))
# ord <- order(inter_var, decreasing = T)
# selected <- gene_info_inc[ord[1:50],4]
...
p1 <- ggplot(data, aes(x=tissue, y=NRGN, fill=pheno)) +
    scale_color_viridis(discrete = TRUE) +
    geom_boxplot() + 
    ggtitle("NRGN expression") +
    xlab("") + ylab("Levels") +
    theme_ipsum(base_family = "Times New Roman", base_size= 12, plot_title_face = "bold", axis_title_size = 16, plot_title_size = 18) +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), plot.margin = unit(c(5, 2, 5, 2), "pt"))

# p1 <- ggplot(data, aes(x=tissue, y=MT3, fill=pheno)) + 
#     scale_color_viridis(discrete = TRUE) +
#     geom_boxplot() + 
#     ggtitle("MT3 expression") +
#     xlab("") + ylab("Levels") +
#     theme_ipsum(base_family = "Times New Roman", base_size= 12, axis_title_size = 14, plot_title_size = 15) +
#     theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), plot.margin = unit(c(5, 2, 5, 2), "pt"))

p2 <- ggplot(data, aes(x=tissue, y=CAMK2A, fill=pheno)) + 
    scale_color_viridis(discrete = TRUE) +
    geom_boxplot() + 
    ggtitle("CAMK2A expression") +
    xlab("") + ylab("Levels") +
    theme_ipsum(base_family = "Times New Roman", base_size= 12, axis_title_size = 14, plot_title_size = 15) +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), plot.margin = unit(c(5, 2, 5, 2), "pt"))

p3 <- ggplot(data, aes(x=tissue, y=SHISA6, fill=pheno)) +
    scale_color_viridis(discrete = TRUE) +
    geom_boxplot() + 
    ggtitle("SHISA6 expression") +
    xlab("") + ylab("Levels") +
    theme_ipsum(base_family = "Times New Roman", base_size= 12, axis_title_size = 14, plot_title_size = 15) +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), plot.margin = unit(c(5, 2, 5, 2), "pt"))
...
id <- 3
cat("tissue name:", structure_info$snames[id], "\n")
...
dotplot(up_reg, font.size = 9, showCategory=30, label_format = 60) +
  theme(text=element_text(family="Times New Roman"))
...
selected <- order(apply(interactions[cfd_id, ], 2, var), decreasing = TRUE)[1:4]
...
dotplot(upreg, font.size = 9, showCategory=30, label_format = 70) + 
  theme(text=element_text(family="Times New Roman"))
...
selected <- order(apply(interactions[cfd_id, ], 2, var), decreasing = TRUE)[1:7]
...
## metagene links cognitive dysfunction
...
## metagene characterize the funtion of brain regions
...
idx <- 17 # metagenes with negative values for white matter of forebrain
cutoffs <- quantile(column_factor[idx, ], probs = seq(0, 1, 0.025))
# selected <- (column_factor[idx, ] <= cutoffs[2])
selected <- (column_factor[idx,] >= cutoffs[length(cutoffs)-1]) # greatest negative difference
...
idx <- 18 # metagenes with positive values only for hippocampus
cutoffs <- quantile(column_factor[idx, ], probs = seq(0, 1, 0.025))
# selected <- (column_factor[idx, ] <= cutoffs[2])
selected <- (column_factor[idx,] >= cutoffs[length(cutoffs)-1]) # greatest negative difference
...
## metagene characterize donors

```{r, fig.height=6, size="small"}

result <- data.frame(meta[,c(4,14)], donor_factor)
t.test(X5 ~ sex, data = result)
pval <- sapply(1:23, function(i) t.test(as.formula(paste0("X", i, "~ sex")), data = result)$p.value)
...
pval <- sapply(1:23, function(i) t.test(as.formula(paste0("X", i, "~ ever_tbi_w_loc")), data = result)$p.value)
