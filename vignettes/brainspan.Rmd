...
setwd("/Users/zhaokai/data/bulk RNA-seq datasets/brainspan_genes_matrix_csv")
...
loadfonts(quiet = T)
result <- data.frame(stage = r_names, levels = stage_factor[, c(2, metagene_id, 17)], stringsAsFactors = F)
colnames(result)[2:4] <- c("metagene_2", "metagene_16", "metagene_17")

long_result <- melt(result, id="stage")
colnames(long_result)[2] <- "metagene"
levels(long_result[[2]]) <- c(2, 16, 17)
long_result$stage <- factor(r_names, levels = r_names)
...
ggplot(data = long_result, aes(x=stage, y=value, colour=metagene, group = metagene)) + 
  geom_line(size = 0.8) +
  scale_x_discrete(labels=2:14)+
  theme_ipsum(base_family = "Times New Roman", base_size= 14, axis_title_size = 16) + 
  xlab("Stages") + ylab("Levels") 

result <- data.frame(stage = r_names, levels = stage_factor[, 14], stringsAsFactors = F)

ggplot(data = result, aes(x = stage, y = levels, group = 1)) +
    geom_line(size = 0.8, color = "darkblue") +
    scale_x_discrete(labels=2:14)+
    xlab("Stages") + ylab("Levels") + ylim(-15,0)+ 
    theme_ipsum(base_family = "Times New Roman", base_size= 14, axis_title_size = 16) + 
  xlab("Stages") + ylab("Levels") 
```
...
down_reg <- enrichGO(gene      = unique(meta[selected,5]),
                     OrgDb     = 'org.Hs.eg.db',
                     ont       = "BP",
                     readable  = TRUE)
...
idx <- 17 # cell defferentiation and DNA replication (up) cognition (down)
...
```{r, fig.height=6, size="small", tidy=TRUE, tidy.opts=list(width.cutoff=I(120))}
...
upreg <- enrichGO(gene = unique(meta[selected,5]),
                  ...
                  ont = "BP",
                  readable = TRUE)
dotplot(upreg, font = 9, showCategory=30, label_format = 80) + 
  theme(text=element_text(family="Times New Roman"))
```

## Metagenes uncover the biological BPs distinct to specific development stage

```{r, fig.height=6, size="small"}
# ------------------ functions of development stage -------------------------
...
idx <- 5  # muscle tissue development
...
idx <- 8 # fatty acid, linoleic acid, steroid, and cholestrol metabolic, immune, nutrition (up), insulin and hormone secretion (down)
...
## Metagenes characterize development of brain regions

```{r, fig.height=6, size="small"}
# ------------------ functions of brain regions -------------------------
idx <- 1 # striatum for cognition, dopamine, monoamine, catecholamines (up)
...
idx <- 3 # mediodorsal nucleus of thalamus for heart contraction
...
idx <- 7 # hippocampal formation for snaptic function and activities
...
idx <- 12 # # cerebellum for eye visual, sensory system 
...
idx <- 13  # cerebellar cortex for kindney function
cutoffs <- quantile(column_factor[idx, ], probs = seq(0, 1, 0.025))
selected <- (column_factor[idx, ] <= cutoffs[2])
